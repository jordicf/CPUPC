...
import numpy as np
from tools.floorset_parser.floor_set_manager.utils.utils import (
    compute_centroid,
    compute_perimeter,
    weight_sum,
    rectangle_decomposition,
)
from cpupc.netlist.yaml_write_netlist import dump_yaml_namededges
from cpupc.utils.utils import write_json_yaml
from cpupc.utils.keywords import KW

from cpupc.geometry.geometry import Point  # check if it is the same
from cpupc.netlist.netlist_types import NamedHyperEdge
...
class FloorSetInstance:
    ...
    def __init__(
        self,
        floorplan_data: dict[str, np.ndarray],
        density: float | None,
        io_soft: bool = False,
    ) -> None:
        """
        ...
        assert isinstance(
            floorplan_data, dict
        ), "Error floorplan data type. Has to be a dict."
        ...
        assert isinstance(
            floorplan_data["area_blocks"], np.ndarray
        ), "Wrong data type for area_blocks"
        assert not (
            floorplan_data["area_blocks"] < 0
        ).any(), "Negative values not allowed in area input"

        assert isinstance(
            floorplan_data["b2b_connectivity"], np.ndarray
        ), "Wrong data type for b2b_connectivity"
        assert not (
            floorplan_data["b2b_connectivity"] < 0
        ).any(), "Negative values not allowed in block-to-block connections"

        assert isinstance(
            floorplan_data["p2b_connectivity"], np.ndarray
        ), "Wrong data type for p2b_connectivity"
        assert not (
            floorplan_data["p2b_connectivity"] < 0
        ).any(), "Negative values not allowed in pin-to-block connections"

        assert isinstance(
            floorplan_data["pins_pos"], np.ndarray
        ), "Wrong data type for pins_pos"
        assert not (
            floorplan_data["pins_pos"] < 0
        ).any(), "Negative values not allowed in pin positions"

        assert isinstance(
            floorplan_data["placement_constraints"], np.ndarray
        ), "Wrong data type for placement constraints"
        assert not (
            floorplan_data["placement_constraints"] < 0
        ).any(), "Negative values not allowed in placement constraints"

        assert isinstance(
            floorplan_data["vertex_blocks"], np.ndarray
        ), "Wrong data type for vertex_blocks"

        assert isinstance(
            floorplan_data["metrics"], np.ndarray
        ), "Wrong data type for metrics"
        assert not (
            floorplan_data["metrics"] < 0
        ).any(), "Negative values not allowed in any metric value"
        ...
        if density:
            assert (
                isinstance(density, float) and 0 <= density <= 1
    ...
    def _parse_modules(self, io_soft: bool) -> None:
        ...
        for mod_id in range(self.num_modules):
            ...
            if len(vertices) > 1:  # Handling Prime FloorSet
                ...
                # is the low-left point. In CPUPC rectangles are stored as [cx,cy,w,h],
                # where c is the center.
                cx = float((vertices[2] + vertices[0]) / 2)
                cy = float((vertices[3] + vertices[1]) / 2)
                data[KW.RECTANGLES] = [cx, cy, float(vertices[0]), float(vertices[1])]
                # c = Point(cx, cy)
            else:
                pass  # This should never happen

            # FloorSet   | CPUPC constraint
            # Pre-placed | fixed
            # Fixed      | hard
    ...
    def _parse_connections(self) -> None:
        ...
        if self._d:
            ...
            for mod_id in range(self.num_modules):
                bl_w = weight_sum(
                    self._fp_data["b2b_connectivity"],
                    self._fp_data["p2b_connectivity"],
                    mod_id,
                )
                vertices = self._fp_data["vertex_blocks"][mod_id]
                vertices = vertices[vertices[:, 0] != -1]
    ...
    def write_yaml_FPEF(self, filename: str | None = None) -> str | None:
        ...
        data = {KW.MODULES: self.modules, KW.NETS: dump_yaml_namededges(self.nets)}
        return write_json_yaml(data, False, filename)

    def write_yaml_DIEF(self, filename: str | None = None) -> str | None:
        """Writes the data into a YAML file. If no file name is given, a string with the yaml contents is returned"""
        data = {KW.WIDTH: self._width, KW.HEIGHT: self._height}
        return write_json_yaml(data, False, filename)
