...
def find_font_file(query: str):
    matches = list(
        filter(lambda path: query in os.path.basename(path), fontman.findSystemFonts())
    )
    return matches
...
def cross_dot_implementation(
    self, point: tuple[float, float], color: str, radius: float
):
...
def thin_cross_dot_implementation(
    self, point: tuple[float, float], color: str, radius: float
):
    width_ratio = (self.x1 - self.x0) / self.width
    height_ratio = (self.y1 - self.y0) / self.height
...
def thin_circle_dot_implementation(
    self, point: tuple[float, float], color: str, radius: float
):
    width_ratio = (self.x1 - self.x0) / self.width
    height_ratio = (self.y1 - self.y0) / self.height
    self._draw_ellipse(
        point, (radius * width_ratio, radius * height_ratio), color, None, 1
    )


def solid_circle_dot_implementation(
    self, point: tuple[float, float], color: str, radius: float
):
    width_ratio = (self.x1 - self.x0) / self.width
    height_ratio = (self.y1 - self.y0) / self.height
    self._draw_ellipse(
        point, (radius * width_ratio, radius * height_ratio), None, color, 1
    )


def solid_line_implementation(
    self,
...
def dashed_line_implementation(
    self,
    line: tuple[tuple[float, float], tuple[float, float]],
    color: str,
    thickness: float,
...
class Canvas:
    ...
    def __init__(self, width: int = 800, height: int = 800):
        ...
        self.clear()
        self.dot_implementations = {
            "cross": cross_dot_implementation,
        ...
        self.line_implementations = {
            "solid": solid_line_implementation,
    ...
    def _draw_ellipse(
        ...
        width_ratio = (self.x1 - self.x0) / self.width
        height_ratio = (self.y1 - self.y0) / self.height
        c, r = self.interpolate(center), (
            radii[0] / width_ratio,
            radii[1] / height_ratio,
        )
    ...
    def portray_changes(self):
        ...
        self.context.rectangle(
            (0, 0, self.width, self.height), fill=(0, 0, 0, 0), outline=None
        )
    ...
    def line(
        ...
        if line_type in self.line_implementations:
            ...
            raise Exception(
                "Line type unknown. Allowed types: "
                + ", ".join(list(self.line_implementations.keys()))
            )
    ...
    def dot(
        ...
        if dot_type in self.dot_implementations:
            ...
            raise Exception(
                "Dot type unknown. Allowed types: "
                + ", ".join(list(self.dot_implementations.keys()))
            )
