...
from cpupc.netlist.netlist_types import Module
from cpupc.geometry.geometry import Point, Shape
from typing import Any


def netlist_to_graph(netlist: Netlist) -> tuple[nx.Graph, dict[int, np.ndarray]]:
    """
    ...
    for net in netlist._edges:
        ...
        if n_mod == 2:  # normal edge
            ...
            # add a dummy vertex
            G.add_node(next_dummy_id, center=center_mass, area=0, dummy=True)
...
def graph_to_netlist(
    netlist: Netlist, G: nx.Graph, positions: dict[int, np.ndarray]
) -> Netlist:
    """
...
def main(prog: str | None = None, args: list[str] | None = None) -> None:
    ...
    positions: dict[int, np.ndarray] = {}
    positions, scaling_factor = forceatlas2_layout(
        G=netlist_graph,
        H=die.height,
        W=die.width,
        max_iter=max_iter,
