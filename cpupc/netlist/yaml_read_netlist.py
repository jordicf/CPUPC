...
def parse_yaml_module(name: str, info: dict[str, Any]) -> Module:
    ...
    assert not (
        KW.IO_PIN in params and KW.TERMINAL in params
    ...
    assert KW.FIXED not in params or isinstance(
        params[KW.FIXED], bool
    ), f"Module {name}: incorrect value for fixed (should be a boolean)"
    assert KW.HARD not in params or isinstance(
        params[KW.HARD], bool
