...
def parse_die(
    ...
    can have the form <width>x<height> (e.g., 5.5x10) or be a JSON/YAML contents.
    :param stream: name of JSON/YAML file or string of text
    ...
    assert (
        KW.WIDTH in tree and KW.HEIGHT in tree
...
def parse_die_rectangle(r: list) -> Rectangle:
    ...
    :return: a Rectangle
    """
    assert isinstance(r, list) and len(r) == 5, f"Incorrect format of die rectangle {r}"
    for i in range(4):
...
def parse_pin_segment(name: str, r: list) -> Rectangle:
    ...
    ), "Die: incorrect format of pin segment for {name}"
    assert all(
