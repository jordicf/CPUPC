...
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # back to the CPUPC
BENCH_EXAM_DIR = (
    PROJECT_ROOT / "tools" / "legalrect" / "test-exam"
)  # locate the test-exam directory
...
class TestLegalizerUnit(unittest.TestCase):
    ...
    def test_model_objective_and_solve(self):
        ...
        model = Model(
            ...
            max_ratio,
            og_names,
            self.options["otol_initial"],
            self.options["tol_decay"],
            self.options["wl_mult"],
